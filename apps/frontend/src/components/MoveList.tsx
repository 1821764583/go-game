...
export default function MoveList({ moves, myColor, currentIndex, onSelect }: MoveListProps) {
  return (
    <div className="bg-gray-800 rounded-xl p-4 h-full overflow-y-auto">
      ...
      {moves.length === 0 ? (
        ...
        <div className="space-y-1">
          {moves.map((m, i) => {
            const active = currentIndex === m.moveNumber;
            return (
              <div
                key={i}
                onClick={onSelect ? () => onSelect(m.moveNumber) : undefined}
                className={`flex items-center gap-2 px-2 py-1 rounded text-xs ${
                  onSelect ? 'cursor-pointer hover:bg-gray-600' : ''
                } ${active ? 'bg-amber-500/30 ring-1 ring-amber-500' : m.player === myColor ? 'bg-gray-700' : ''}`}
              >
